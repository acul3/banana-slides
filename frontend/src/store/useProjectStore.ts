  ...
  return {
      ...
      try {
        ...
        if (error.response) {
          ...
          if (error.response.status === 404) {
            ...
            errorMessage = errorData?.error?.message || 'Project not found, may have been deleted';
    ...
    startAsyncTask: async (apiCall) => {
      ...
      try {
        ...
        set({ error: error.message || 'Failed to start task', isGlobalLoading: false });
        throw error;
      }
    },
    ...
    pollTask: async (taskId) => {
      ...
      const poll = async () => {
        try {
          ...
          set({
            error: normalizeErrorMessage(error.message || 'Failed to query task status'),
            activeTaskId: null,
            isGlobalLoading: false
          });
        }
            ...
            if (task) {
              ...
              if (task.status === 'COMPLETED') {
                ...
                set({
                  ...
                  taskProgress: null,
                  activeTaskId: null,
                  error: normalizeErrorMessage(task.error_message || task.error || 'Failed to generate description')
                });
              } else if (task.status === 'PENDING' || task.status === 'PROCESSING') {
                // 继续轮询
    ...
    generatePageImage: async (pageId, forceRegenerate = false) => {
      ...
      try {
        ...
        delete newTasks[pageId];
        set({ pageGeneratingTasks: newTasks, error: normalizeErrorMessage(error.message || 'Failed to generate image') });
        throw error;
      }
    },
        ...
        try {
          ...
          if (task.status === 'COMPLETED') {
            ...
            set({
              ...
              error: normalizeErrorMessage(task.error_message || task.error || 'Generation failed')
            });
            // 刷新项目数据以更新页面状态
            await get().syncProject();
          } else if (task.status === 'PENDING' || task.status === 'PROCESSING') {
    ...
    editPageImage: async (pageId, editPrompt, contextImages) => {
      ...
      try {
        ...
        const newTasks = { ...pageGeneratingTasks };
        delete newTasks[pageId];
        set({ pageGeneratingTasks: newTasks, error: normalizeErrorMessage(error.message || 'Failed to edit image') });
        throw error;
      }
    },
