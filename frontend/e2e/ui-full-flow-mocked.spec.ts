...
test.describe('UI-driven E2E test (Mocked Backend)', () => {
  ...
  test('User Full Flow: Create and export PPT with mocked API', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("Batch image generation")', { timeout: 10000 })
    const generateImageBtn = page.locator('button:has-text("Batch image generation")')
